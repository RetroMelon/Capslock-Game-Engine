...
public abstract class game {
	 ...
	 * it includes a set of methods called "game<method>" which are private and non-abstract. these methods are designed to do some of the basic features
	 ...
	 * 
	 * the main elements inside the main loop (input, update, draw) are available top be overridden, and are called input, update and draw.
	 ...
	 * 
	 * the pausing of the game is handled by the input method, however the unpausing of the game is handled by the loop that executes while the game is paused
	 *  
	 */
	...
	protected int quittype = 1; // can be used if the game is going to do some kind of animation on quitting
	protected boolean paused = false;
	protected boolean breakpause = false;//used by the pauseaction method if it wants to break the pause cycle
	
	
	public game(EngineInstance e){
		setupEngine(e);
	}
	public game(EngineInstance e, boolean startoncreate){//gives the option to start the game as soon as it is created. this does not allow for the returning of a quittype integer. so should only be used for basic game types
		setupEngine(e);
	...
	public Object run(){
		...
		while(!quit){
			if(paused){
				while(mainscreen.keyboard.keyPressed(KeyEvent.VK_ESCAPE) && !breakpause){pauseaction();}//the key will still be pressed down.
				while(!mainscreen.keyboard.keyPressed(KeyEvent.VK_ESCAPE) && !breakpause){pauseaction();try{Thread.sleep(2);}catch(Exception e){}}//the key is now no longer pressed down
				while(mainscreen.keyboard.keyPressed(KeyEvent.VK_ESCAPE) && !breakpause){pauseaction(); try{Thread.sleep(2);}catch(Exception e){}}//the key has been pressed down again, meaning it is about to be released.  
				breakpause=false;
				unpause();
			}
	...
	protected void updateremoveentities(){
		try{entities.removeAll(removelist);}catch(Exception e){e.printStackTrace();}
		removelist.clear();
	}
