...
public class peffect extends entity implements Runnable{
	...
	public void draw(){
		if(drawable){
			...
			mainscreen.screengraphics.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, transparency));
	...
	private class fader implements Runnable{
		...
		public void run() {
			try{
				...
				int iterations = (int)(  (double)lifespan   /   (double)timeinterval   *   (100-percentagetimebeforefade)/100   );
				
				//sleeping until we are required to start fading
				try{Thread.sleep((int)(((double)percentagetimebeforefade/100)*lifespan));}catch(Exception e){}
