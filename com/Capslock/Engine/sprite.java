...
public class sprite {
	...
	public boolean draw(Graphics2D output, int x, int y, double transparency) {
		try {
			...
			output.drawImage(getimage(), x, y, null);
			output.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 1));//setting the composite setting back to normal, so that we don't accidentally draw other things with a transparency.
			return true;
		} catch (Exception e) {}
		return false;
	}	
